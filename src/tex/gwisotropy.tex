...
\footnote{These three-dimensional quantiles correspond to the fraction of posterior samples with higher probability density than the origin.  \wf{I think it matters, so is this density in $\vec{v}$ or $\vec{u}$ space?}}
...
\footnote{For example, it is easy to see that this would be the case if the posterior samples for $\dip{J/N}$ where drawn from a zero-mean multivariate Gaussian, irrespective of the variance.  \wf{I don't undestand this footnote: if, in fact, the variance tensor of this supposed Gaussian were isotropic (proportional to the identity), then the samples \emph{would} be uniformly distributed on the sky, no?}}
