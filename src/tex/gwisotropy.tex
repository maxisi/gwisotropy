...
% \footnote{It is easy to see that this would be the case for any finite number of draws from a zero-mean, multivariate and isotropic Gaussian, irrespective of the variance. 
% %% WIP: ongoing discussion about Will's comment below:
% % \wf{I don't undestand this footnote: if, in fact, the variance tensor of this supposed Gaussian were isotropic (proportional to the identity), then the samples \emph{would} be uniformly distributed on the sky, no?}
% }
