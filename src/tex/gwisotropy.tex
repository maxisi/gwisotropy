...
\author[0000-0001-8830-8672]{Maximiliano Isi}
...
\author[0000-0002-9994-1761]{Vijay Varma}
...
This differs from previous works like \citet{Vitale:2022pmu}, which checked for anomalies in the alignment of sources with respect to Earth, as reflected in the distribution of \ac{BBH} inclinations relative to the line of sight (Fig.~\ref{fig:vectors}, third panel).
...
In order to study the spatial distribution and orientation of \acp{BBH}, we must analyze the collection of detections under a hierarchical population model that allows for variable degrees of spatial and directional correlations \citep[see, e.g.,][]{Essick:2022slj}.
...
We reweight those samples following the astrophysical population described above \citep[see, e.g.,][for methodology]{Miller:2020zox,Essick:2022ojx} and then use them to produce distributions for the components of $\hat{N}$ and $\hat{J}$, which we use as input for our hierarchical analysis based on Eq.~\eqref{eq:lnlike}.
...
\software{\textsc{PyMC} \citep{thomas_wiecki_2023_7868623}, \textsc{ArviZ} \citep{osvaldo_a_martin_2023_7703176}, \textsc{NumPy} \citep{Harris:2020xlr}, \textsc{SciPy} \citep{Virtanen:2019joe}, \textsc{matplotlib} \citep{Hunter:2007}, \textsc{ligo.skymap} \citep{skymap,Singer:2016eax,Singer:2016erz}, \showyourwork \citep{Luger2021}.}
...
The skymaps were produced using the \textsc{ligo.skymap} package \citep{skymap,Singer:2016eax,Singer:2016erz}, a set of standard LIGO-Virgo tools for the processing of probability densities over the sphere.
We make these figures, corresponding \ac{FITS} files, and code used to generate them, available in our data release \citep{repo}.
